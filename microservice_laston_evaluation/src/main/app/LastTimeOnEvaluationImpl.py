...
class LastTimeOnEvaluation(object):
    ...
    def last_time_on_evaluation(self, user_id, rule_id, device_id):
        ...
        # time now
        measure_now_str = datetime.now().strftime("%H:%M:%S")
        measure_now = datetime.strptime(measure_now_str, '%H:%M:%S')
        # delta time last on
        device_last_on = self.r.get("device:" + device_id + ":last_on")
        measure_time_last_on = datetime.strptime(device_last_on, '%H:%M:%S')
        time_delta_on = (measure_now - measure_time_last_on).total_seconds()
        # delta time last off
        device_last_off = self.r.get("device:" + device_id + ":last_off")
        measure_time_last_off = datetime.strptime(device_last_off, '%H:%M:%S')
